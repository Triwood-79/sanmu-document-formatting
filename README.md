...
| 表格、图片和文本框会怎样？ | 原样保留，Skill 会提醒你人工检查，不会擅自改动其中的内容。 |
...
没有提供的署名、单位、日期、版记或事实内容，Skill 不会自行补写。
...
修改时，Codex 会按页面、字体、标题、正文、说明、版记和页码等内容分组提问。保存前会展示“当前值 → 新值”的差异；没有收到 `确认保存`，原来的格式就继续生效。
...
| 版记 | 方正仿宋_GBK，三号 16 pt，只处理用户提供的内容 |
| 打印模式 | 默认双面打印 |
...
- 表格、图片、文本框和特殊横向页面会保留，但不自动统一内部格式。
